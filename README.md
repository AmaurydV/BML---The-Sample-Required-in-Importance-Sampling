...
You can also run the code directly in a Python session or script from the repository root.

Example:

```python
import numpy as np
from Independent_spins.Independent_spins import experiment

N_list = [2, 10]
beta0 = 1
beta = 0.5
c_grid = np.arange(-1, 1.5, 0.5)
reps = 20
seed = 42
...
print(results_iid[2]["L"])
print(results_iid[2]["by_c"][0.0])
```

Important:
- run commands from the **repository root** so Python can import `Independent_spins`, `ideal_case`, and `Mixture_bimodale` correctly,
- or add the repository path to `PYTHONPATH`.

---

## Outputs produced by the code

The code mainly produces:
- in-memory Python dictionaries containing experiment summaries,
- matplotlib figures displayed from the notebook,
- printed estimates of `L` for some experiments.

It does **not** currently save plots or result tables to disk automatically.
If you want saved outputs, you can add lines such as:
...
or export results manually with `json`, `pickle`, or `pandas`.

---

## Notes on reproducibility

- The experiments use explicit random seeds.
- Re-running with the same seed should reproduce the same stochastic results, up to possible small differences across Python / NumPy versions.
- The project contains `__pycache__` files, but they are not needed and can be ignored.

---

## Suggested cleanup / improvements

To make the repository easier to reuse, you may later want to add:
- a `requirements.txt` file,
- a `environment.yml` file,
- automatic saving of figures,
- a script version of the notebook,
- a short description of the theoretical background in the README.
...
cd BML---The-Sample-Required-in-Importance-Sampling
python3 -m venv .venv
source .venv/bin/activate
pip install numpy matplotlib jupyter
jupyter notebook
...
Then open **`Run_Experiments.ipynb`** and run all cells.

That reproduces the full set of experiments currently included in the repository.
